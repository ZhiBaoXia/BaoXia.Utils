...
[TestClass]
public class ItemsConcurrentProcessQueue2Test
{
	[TestMethod]
	public async Task ProcessItem_ShouldUseHandlerOfCurrentItem()
	{
		var processQueue = new ItemsConcurrentProcessQueue2<int>(1);
		...
		processQueue.ProcessItem(1, (item) =>
		{
			...
			canFirstItemProcessCompleted.Wait(TimeSpan.FromSeconds(5));
		});
		try
		{
	...
	public async Task WhenAll_ShouldAwaitAsyncHandlersAndLimitConcurrency()
	{
		...
		var itemsCount = 200;
		var processQueue = new ItemsConcurrentProcessQueue2<int>(concurrencyCountMax);
	...
	{
		var processQueue = new ItemsConcurrentProcessQueue2<int>(4);
