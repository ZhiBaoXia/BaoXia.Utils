...
public class ItemsIndexWith4Keys<ItemType, PrimaryIndexKeyType, SecondaryIndexKeyType, ThirdaryIndexKeyType, FourthIndexKeyType>(
		...
		Func<ItemType[], ItemType[]?> toUpdateIndexItems)
	{
		...
		var fourthIndexes
			= thirdaryIndexes.GetOrAdd(
				thirdaryIndexKey,
				(_) => []);
		var itemIndexInfo = fourthIndexes.GetOrAdd(
			fourthIndexKey,
			(_) => new());
		lock (itemIndexInfo)
		{
