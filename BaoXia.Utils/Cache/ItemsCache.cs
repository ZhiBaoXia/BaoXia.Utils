...
namespace BaoXia.Utils.Cache
{
                        ...
                        where ItemKeyType : notnull
        {
                        ...
                        CancellationToken cancellationToken)
                {
                        ...
                        if (noneUpdateSecondsToUpdateItemCache > 0.0)
                        {
                                this.DidUpdateAllItemCache(noneUpdateSecondsToUpdateItemCache, cancellationToken);
                        }
                }

                public void UpdateAllItemCache(CancellationToken cancellationToken)
                {
                        this.UpdateAllItemCache(this.NoneUpdateSecondsToUpdateItemCache, cancellationToken);
                }
                                        ...
                                        if (itemContainer.IsItemValid != true)
                                        {
                                                ...
                                                else if (this.ToDidCreateItemCache != null)
                                                {
                                                        if (DidWillCreateItemCache(
                                                                ...
                                                                out var newItem))
                                                        {
                                                                newItem = this.ToDidCreateItemCache(
                                                                        itemCacheKey,
                                                                        itemCacheCreateParam);
                                                        }
                                                        if (newItem != null
                ...
                protected virtual ItemType? DidItemCacheUpdated(
                        ItemKeyType itemKey,
                        ItemType? lastItem,
