...
public class NotificationQueue : ConcurrentQueue<Notification>
{
	    ...
	    CancellationToken cancellationToken)
	{
		...
		while (cancellationToken.IsCancellationRequested != true)
		{
				...
				var taskToSendNotification = tasks.RunAsync(async () =>
				{
					try
					{
						...
						if (notificationProcessTimeoutSeconds > 0)
						{
							...
							{
								int notificationProcessTimeoutMilliseconds = (int)(1000.0 * notificationProcessTimeoutSeconds);
								cancellationTokenSourceWithTimeout.CancelAfter(notificationProcessTimeoutMilliseconds);
							}
						...
						{
							notificationSendResult = toSendReceivedNotification(notification, cancellationTokenToSendNotification);
						}
						else if (toSendReceivedNotificationAsync != null)
						{
							notificationSendResult = await toSendReceivedNotificationAsync(notification, cancellationTokenToSendNotification);
						}
					...
					finally
					{
						...
						{
							var toNotificationSended = notification.ToNotificationSended;
							var toNotificationSendedAsync = notification.ToNotificationSendedAsync;
						...
						//		"消息发送数超出了预期，" + "预期“" + (objectNotificationListenersCount) + "”个，" + "实际“" + (-1 * sendedObjectNotificationListenersCount) + "”个。");
						//}
					}
				});
				// !!!
			...
			{
				Task.WaitAll([.. tasksToSendNotification], cancellationToken);
			}
		}
		return this.Count;
	}
